...
class SUMOpipeline:
    ...
    def covert_to_sumo(
        self,
    ):  # Function to convert osm file to .net.xml (sumo network file)
        cmd = [
            "netconvert",
            "--osm-files",
            self.osm_file,
    ...
    def generate_demand(
        self, vehicle_count
    ):  # Function to generate demand and write it to the .rou.xml file
...
class SUMOsimulation:
    ...
    def print_summary(
        self,
    ):  # Function to print summary of results from the summary file
...
def main():
    ...
    sim = SUMOsimulation(
        args.name, network.net_file, network.rou_file, network.output_dir
    )
    sim.write_config()
    sim.run_simulation()
