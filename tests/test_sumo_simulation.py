...
def test_write_config_and_cfg_content(tmp_path):
    out = tmp_path
...
def test_print_summary_parses_stats(tmp_path):
    out = tmp_path
...
def test_run_simulation_invokes_sumo_and_prints_summary(monkeypatch, tmp_path):
    out = tmp_path
    net = out / "test.net.xml"
    rou = out / "test.rou.xml"
    net.write_text("")
