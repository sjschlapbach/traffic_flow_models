...
class TestCell:
    def test_init_assigns_attributes(self):
        ...
        link.partition_link(preferred_cell_size=2.5, dt=0.001)
    ...
    def test_partition_link_creates_cells_of_preferred_size_and_remainder(self):
        link = MotorwayLink(
            length=3.5, lanes=2, lane_capacity=1500, free_flow_speed=90, jam_density=140
        )
        preferred = 1.0
        dt = 0.01

        # compute expected partitioning using the same rules as the implementation
        min_cell_length = link.vf * dt
        ...
        link.partition_link(preferred_cell_size=preferred, dt=dt)
