...
def test_init_creates_output_dir(tmp_path, monkeypatch):
...
def test_skip_if_exists_decorator_skips(monkeypatch, tmp_path, capsys):
    monkeypatch.chdir(tmp_path)
...
def test_covert_to_sumo_runs_netconvert(monkeypatch, tmp_path):
    monkeypatch.chdir(tmp_path)
...
def test_generate_demand_handles_missing_sumo_home(monkeypatch, tmp_path, capsys):
    monkeypatch.chdir(tmp_path)
...
def test_generate_demand_invokes_randomTrips(monkeypatch, tmp_path):
    monkeypatch.chdir(tmp_path)
...
def test_covert_to_sumo_skips_when_net_exists(monkeypatch, tmp_path, capsys):
    monkeypatch.chdir(tmp_path)
