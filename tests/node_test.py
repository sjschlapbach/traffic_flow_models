...
class TestNode:
    ...
    def test_add_and_validate_correct_types(self):
        ...
        m_in = MotorwayLink(length=1.0, lanes=1)
        on = Onramp(length=0.5, lanes=1)
        org = Origin()

        m_out = MotorwayLink(length=1.0, lanes=1)
        off = Offramp(lanes=1)
